...
class _StudentEditTabState extends State<StudentEditTab> {
  ...
  void initState() {
    ...
    _valor = TextEditingController(text: widget.student?['name'] ?? '');
  ...
  void didUpdateWidget(covariant StudentEditTab oldWidget) {
    ...
    if (widget.student != oldWidget.student) {
      ...
      _valor.text = widget.student?['name'] ?? '';
  ...
  void _handleSave() async{
    if (_formKey.currentState!.validate()) {
      await updateNota(id: int.parse(_idCtrl.text),value: double.parse(_valor.text), bearerToken: Provider.of<UserProvider>(context,listen:false).user!.token,);
