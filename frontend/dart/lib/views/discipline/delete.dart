...
import 'package:musa/services/disciplina/deleteDisciplina.dart';
import 'package:musa/services/disciplina/getDisciplina.dart';
import 'package:provider/provider.dart';

class DisciplineDeleteTab extends StatefulWidget {
  final Map<String, dynamic>? student;

  const DisciplineDeleteTab({
    Key? key,
...
class _StudentEditTabState extends State<DisciplineDeleteTab> {
  ...
  @override
  void didUpdateWidget(covariant DisciplineDeleteTab oldWidget) {
  ...
  void _handleSave() async{
    if (_formKey.currentState!.validate()) {
      await deleteDisciplina(id:  _idCtrl.text, token: Provider.of<UserProvider>(context,listen:false).user!.token);
      await getDisciplinas(token: Provider.of<UserProvider>(context,listen:false).user!.token, context: context);
      
  
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Diciplina deletada com sucesso!')),
      );
    }
  }
